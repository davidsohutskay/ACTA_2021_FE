...
int main(int argc, char *argv[])
{
	...
	double t_rho = 4*0.005/1000; // 0.0045/rho_phys force of fibroblasts in MPa, this is per cell. so, in an average sense this is the production by the natural density See Koppenol
	double t_rho_c = 4*0.05/1000; // 0.045/rho_phys force of myofibroblasts enhanced by chemical, I'm assuming normalized chemical, otherwise I'd have to add a normalizing constant
	...
	double D_rhoc = -1.66e-12*10/c_max/c_max; // diffusion of chemotactic gradient, an order of magnitude greater than random walk [mm^2/hour], not normalized
	double D_cc = 0.01208; // 0.15 diffusion of chemical TGF, not normalized. Updated from Murphy
	...
	double p_c_thetaE = 90e-10/rho_phys; // 300.0e-16/rho_phys coupling of elastic and chemical, three fold
    ...
    double time_step_ratio = 500; // time step ratio between local and global (explicit)
    ...
    std::cout<<"Created the surface mesh with "<<myMesh.n_surf_elements<<" elements\n";
