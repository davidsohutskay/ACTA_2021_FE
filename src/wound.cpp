        ...
        VectorXd &Re_c,MatrixXd &Ke_c_x,MatrixXd &Ke_c_rho,MatrixXd &Ke_c_c)
{
    ...
    double t_rho = global_parameters[4]; // force of fibroblasts
    double t_rho_c = global_parameters[5]; // force of myofibroblasts enhanced by chemical
    double K_t = global_parameters[6]; // saturation of collagen on force
    double K_t_c = global_parameters[7]; // saturation of chemical on force
    double D_rhorho = global_parameters[8]; // diffusion of cells
    double D_rhoc = global_parameters[9]; // diffusion of chemotactic gradient
    double D_cc = global_parameters[10]; // diffusion of chemical
    double p_rho =global_parameters[11]; // production of fibroblasts naturally
    double p_rho_c = global_parameters[12]; // production enhanced by the chem
    double p_rho_theta = global_parameters[13]; // mechanosensing
    double K_rho_c= global_parameters[14]; // saturation of cell production by chemical
    double K_rho_rho = global_parameters[15]; // saturation of cell by cell
    double d_rho = global_parameters[16] ;// decay of cells
    double vartheta_e = global_parameters[17]; // physiological state of area stretch
    double gamma_theta = global_parameters[18]; // sensitivity of heviside function
    double p_c_rho = global_parameters[19];// production of C by cells
    double p_c_thetaE = global_parameters[20]; // coupling of elastic and chemical
    double K_c_c = global_parameters[21];// saturation of chem by chem
    double d_c = global_parameters[22]; // decay of chemical
    double bx = global_parameters[23]; // body force
    double by = global_parameters[24]; // body force
    double bz = global_parameters[25]; // body force
    ...
    for(int ip=0;ip<IP_size;ip++)
    {
        ...
        localWoundProblemExplicit(dt,local_parameters,c,rho,FF,ip_phif_0[ip],ip_a0_0[ip],ip_s0_0[ip],ip_n0_0[ip],ip_kappa_0[ip],ip_lamdaP_0[ip],ip_phif[ip],ip_a0[ip],ip_s0[ip],ip_n0[ip],ip_kappa[ip],ip_lamdaP[ip],dThetadCC,dThetadrho,dThetadc);
        ...
        double kv = 1/3.65273;
        double Psivol = 0.5*phif*pow(kv*(Je-1.),2) - 2*phif*k0*log(Je); //*phif
        double dPsivoldJe = phif*kv*(Je-1.) - 2*phif*k0/Je;
        double dPsivoldJedJe = phif*kv + 2*phif*k0/(Je*Je);
        Matrix3d SSe_vol = dPsivoldJe*Je*CCeinv/2; // = phif*kv*Je*(Je-1.)*CCeinv/2 - phif*k0*CCeinv;
        ...
        Vector3d Q_rho = -3.0*(D_rhorho-phif*(D_rhorho-D_rhorho/10))*A0*Grad_rho/trA - 3*(D_rhoc-phif*(D_rhoc-D_rhoc/10))*rho*A0*Grad_c/trA;
...
//        double h_el = pow(8*Jac, 1/3); // element size parameter
//        double a_norm = (-3*(D_rhoc-phif*(D_rhoc-D_rhoc/10))*rho*A0*Grad_c/trA).norm(); // rough norm of the convective velocity
//        double s_norm = (p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He - d_rho); // rough norm of the source term coefficient
//        double D_norm = (-3.0*(D_rhorho-phif*(D_rhorho-D_rhorho/10))*A0/trA).norm(); // rough norm of the diffusive term coefficient
//        double u_max = *max_element(node_rho.begin(),node_rho.end()); // max cell value
//        double tau = 1/((4*D_norm/(h_el*h_el)) + (2*a_norm/h_el) + s_norm*(1-u_max));
//        double Pe = a_norm*h_el/2*D_norm;
//        double Da = s_norm*h_el*h_el/D_norm;
//        // Gradient of S_rho for the SSPG or GGLS
//        Vector3d Grad_S_rho = (p_rho_c*Grad_c/(K_rho_c+c) - p_rho_c*c*Grad_c/((K_rho_c+c)*(K_rho_c+c)))*(rho - rho*rho/K_rho_rho)
//                + (p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He)*(Grad_rho - 2*rho*Grad_rho/K_rho_rho) - d_rho*Grad_rho;
        ...
        for(int nodei=0;nodei<elem_size;nodei++){
            ...
            //Re_rho(nodei) += Jac*tau*(((Grad_rho-Grad_rho0)/dt - Grad_S_rho).dot(node_x[ip]-node_x[nodei])*R[nodei])*wip;
            // GGLS stabilization
            // NOT CURRENTLY FUNCTIONAL
...
//        ip_dphifdc.push_back(dphifdc);
        ...
        Vector3d Grad_S_rho_plus,Grad_S_rho_minus;
        //
        // phif
        evalFluxesSources(global_parameters,phif_plus,a0,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif_minus,a0,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhodphif = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSdphif: \n" << dSSdphif << "\n dQ_rhodphif: \n" << dQ_rhodphif << "\n dQ_cdphif: \n" << dQ_cdphif << "\n dS_rhodphif: \n" << dS_rhodphif << "\n dS_cdphif: \n" << dS_cdphif << "\n";
        //
        // a0x
        evalFluxesSources(global_parameters,phif,a0_plus_x,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0_minus_x,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhoda0x = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSda0x: \n" << dSSda0x << "\n dQ_rhoda0x: \n" << dQ_rhoda0x << "\n dQ_cda0x: \n" << dQ_cda0x << "\n dS_rhoda0x: \n" << dS_rhoda0x << "\n dS_cda0x: \n" << dS_cda0x << "\n";
        //
        // a0y
        evalFluxesSources(global_parameters,phif,a0_plus_y,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0_minus_y,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhoda0y = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSda0y: \n" << dSSda0y << "\n dQ_rhoda0y: \n" << dQ_rhoda0y << "\n dQ_cda0y: \n" << dQ_cda0y << "\n dS_rhoda0y: \n" << dS_rhoda0y << "\n dS_cda0y: \n" << dS_cda0y << "\n";
        //
        // a0z (assume not all principal fiber directions are tangent to surface)
        evalFluxesSources(global_parameters,phif,a0_plus_z,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0_minus_z,a0_0,s0_0,n0_0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhoda0z = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSda0z: \n" << dSSda0z << "\n dQ_rhoda0z: \n" << dQ_rhoda0z << "\n dQ_cda0z: \n" << dQ_cda0z << "\n dS_rhoda0z: \n" << dS_rhoda0z << "\n dS_cda0z: \n" << dS_cda0z << "\n";
        //
        // kappa
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa_plus,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa_minus,lamdaP,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhodkappa = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSdkappa: \n" << dSSdkappa << "\n dQ_rhodkappa: \n" << dQ_rhodkappa << "\n dQ_cdkappa: \n" << dQ_cdkappa << "\n dS_rhodkappa: \n" << dS_rhodkappa << "\n dS_cdkappa: \n" << dS_cdkappa << "\n";
        //
        // lamdaP_a
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa,lamdaP_plus_a,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa,lamdaP_minus_a,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhodlamdaPa = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSdlamdaPa: \n" << dSSdlamdaPa << "\n dQ_rhodlamdaPa: \n" << dQ_rhodlamdaPa << "\n dQ_cdlamdaPa: \n" << dQ_cdlamdaPa << "\n dS_rhodlamdaPa: \n" << dS_rhodlamdaPa << "\n dS_cdlamdaPa: \n" << dS_cdlamdaPa << "\n";
        //
        // lamdaP_s
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa,lamdaP_plus_s,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa,lamdaP_minus_s,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhodlamdaPs = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSdlamdaPs: \n" << dSSdlamdaPs << "\n dQ_rhodlamdaPs: \n" << dQ_rhodlamdaPs << "\n dQ_cdlamdaPs: \n" << dQ_cdlamdaPs << "\n dS_rhodlamdaPs: \n" << dS_rhodlamdaPs << "\n dS_cdlamdaPs: \n" << dS_cdlamdaPs << "\n";
        //
        // lamdaP_N (assume change in height of wound)
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa,lamdaP_plus_n,FF,rho,c,Grad_rho,Grad_c, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus,Grad_S_rho_plus);
        evalFluxesSources(global_parameters,phif,a0,a0_0,s0_0,n0_0,kappa,lamdaP_minus_n,FF,rho,c,Grad_rho,Grad_c, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus,Grad_S_rho_minus);
        ...
        //Vector3d dGrad_S_rhodlamdaPn = (1./(2.*epsilon))*(Grad_S_rho_plus-Grad_S_rho_minus);
        //std::cout << "\n dSSdlamdaPn: \n" << dSSdlamdaPn << "\n dQ_rhodlamdaPn: \n" << dQ_rhodlamdaPn << "\n dQ_cdlamdaPn: \n" << dQ_cdlamdaPn << "\n dS_rhodlamdaPn: \n" << dS_rhodlamdaPn << "\n dS_cdlamdaPn: \n" << dS_cdlamdaPn << "\n";
        //---------------------------------//
        ...
        double Psif14 = 2*k2*kappa*(1-3*kappa)*Psif + 2*k2*kappa*(kappa*I1e + (1-3*kappa)*I4e -1)*Psif4; // I4e
...
//        std::cout<<DDtot<<"\nDD_tot_num\n"<<DDtot_num<<"\n";
        ...
        // GRADIENT OF SOURCE
        // explicit derivatives of gradient of source term
//        Vector3d dGrad_S_rhodrho = (p_rho_c*Grad_c/(K_rho_c+c) - p_rho_c*c*Grad_c/((K_rho_c+c)*(K_rho_c+c)))*(1 - 2*rho/K_rho_rho)
//                                   + (p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He)*(-2*Grad_rho/K_rho_rho);
//        Matrix3d dGrad_S_rhodGradrho = ((p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He)*(1 - 2*rho/K_rho_rho) - d_rho)*Matrix3d::Identity();
//        Vector3d dGrad_S_rhodc = (-p_rho_c*Grad_c/((K_rho_c+c)*(K_rho_c+c)) - p_rho_c*Grad_c/((K_rho_c+c)*(K_rho_c+c)) + 2*p_rho_c*c*Grad_c/((K_rho_c+c)*(K_rho_c+c)*(K_rho_c+c)))*(rho - rho*rho/K_rho_rho)
//                                 + (p_rho + p_rho_c/(K_rho_c+c) - p_rho_c*c/((K_rho_c+c)*(K_rho_c+c)) + p_rho_theta*He)*(Grad_rho - 2*rho*Grad_rho/K_rho_rho);
//        Matrix3d dGrad_S_rhodGradc = (p_rho_c/(K_rho_c+c) - p_rho_c*c/((K_rho_c+c)*(K_rho_c+c)))*(rho - rho*rho/K_rho_rho)*Matrix3d::Identity();
        ...
        std::vector<double> dGrad_S_rhodCC;
        for(int ii=0;ii<3;ii++) {
            for(int jj=0;jj<3;jj++) {
                for(int kk=0;kk<3;kk++) {
                    for(int ll=0;ll<3;ll++) {
                        // These are third order tensors, but there is a contraction from matrix multiplication
                    ...
                    // GRADIENT OF SOURCE
                    // No contraction needed here
                    //dGrad_S_rhodCC[ii*3+jj+kk] += (p_rho_theta*dHedCC_explicit(ii,jj))*(Grad_rho(kk) - 2*rho*Grad_rho(kk)/K_rho_rho);
                }
            }
        }
...
//        }
        ...
        // SSPG Stabilization
        // Need to calculate derivatives as before
//      Vector3d dGrad_S_rhodrho_explicit = (p_rho_c*Grad_c/(K_rho_c+c) - p_rho_c*c*Grad_c/((K_rho_c+c)*(K_rho_c+c)))*(1 - 2*rho/K_rho_rho)
//                    + (p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He)*(Grad_rho - 2*rho*Grad_rho/K_rho_rho) - d_rho*Grad_rho;
//      Vector3d dGrad_S_rhodrho_explicit = (p_rho_c*Grad_c/(K_rho_c+c) - p_rho_c*c*Grad_c/((K_rho_c+c)*(K_rho_c+c)))*(1 - 2*rho/K_rho_rho)
//                    + (p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He)*(Grad_rho - 2*rho*Grad_rho/K_rho_rho) - d_rho*Grad_rho;
//        // total derivatives
//        Vector3d dGrad_S_rhodrho = dS_rhodrho_explicit + dS_rhodphif*dphifdrho + dS_rhoda0x*da0xdrho + dS_rhoda0y*da0ydrho + dS_rhoda0z*da0zdrho
//                            +dS_rhodkappa*dkappadrho+dS_rhodlamdaPa*dlamdaP_adrho+dS_rhodlamdaPs*dlamdaP_sdrho+dS_rhodlamdaPn*dlamdaP_ndrho;
//        Vector3d dGrad_S_rhodc = dS_rhodc_explicit + dS_rhodphif*dphifdc + dS_rhoda0x*da0xdc + dS_rhoda0y*da0ydc + dS_rhoda0z*da0zdc
//                          +dS_rhodkappa*dkappadc+dS_rhodlamdaPa*dlamdaP_adc+dS_rhodlamdaPs*dlamdaP_sdc+dS_rhodlamdaPn*dlamdaP_ndc;
        ...
        for(int nodei=0;nodei<elem_size;nodei++){
            for(int nodej=0;nodej<elem_size;nodej++){
                for(int coordj=0;coordj<3;coordj++){
                    ...
                    // SSPG stabilization
                    // If IP == nodei, the term disappears
                    // If not, d/du of nodex[ip]-nodex[nodei] = -Identity[nodei,nodej]
                    //Ke_rho_x(nodei,nodej*3+coordj) += Jac*tau*((- dGrad_S_rhodCC*linCC_voigt)*(node_x[ip]-node_x[nodei]) + Grad_S_rho[nodei]*(-Matrix3d::Identity()(nodei,nodej))*R[nodei])*wip;
                ...
                // SSPG stabilization
                //Ke_rho_rho(nodei,nodej) += Jac*tau*((Grad_R[nodej]/dt - (dGrad_S_rhodrho + dGrad_S_rhodGradrho*Grad_R[nodej])).dot(node_x[ip]-node_x[nodei])*R[nodei])*wip;
                ...
                // SSPG stabilization
                //Ke_rho_c(nodei,nodej) += Jac*tau*((-dGrad_S_rhodc + dGrad_S_rhodGradc*Grad_R[nodej]).dot(node_x[ip]-node_x[nodei])*R[nodei])*wip;
...
void evalFluxesSources(const std::vector<double> &global_parameters, const double& phif,Vector3d a0, const Vector3d& a0_0, const Vector3d& s0_0, const Vector3d& n0_0,double kappa, const Vector3d& lamdaP,
                       ...
                       Matrix3d & SS,Vector3d &Q_rho,double &S_rho, Vector3d &Q_c,double &S_c, Vector3d &Grad_S_rho)
{
    double kv = global_parameters[0]; // incompressibility penalty
    double k0 = global_parameters[1]; // neo hookean
    double kf = global_parameters[2]; // stiffness of collagen
    ...
    // Instead of (double pressure = -k0*lamda_N*lamda_N;) directly, add volumetric part of stress SSvol
    kv = 1/3.65273;
    double Psivol = 0.5*phif*pow(kv*(Je-1.),2) - 2*phif*k0*log(Je);
    double dPsivoldJe = phif*kv*(Je-1.) - 2*phif*k0/Je;
    ...
    //------------------//
    // Gradient of S_rho for the SSPG or GGLS
    Grad_S_rho = (p_rho_c*Grad_c/(K_rho_c+c) - p_rho_c*c*Grad_c/((K_rho_c+c)*(K_rho_c+c)))*(rho - rho*rho/K_rho_rho)
                          + (p_rho + p_rho_c*c/(K_rho_c+c) + p_rho_theta*He)*(Grad_rho - 2*rho*Grad_rho/K_rho_rho) - d_rho*Grad_rho;
}
...
void evalSS(const std::vector<double> &global_parameters, double phif, Vector3d a0, Vector3d s0, Vector3d n0, double kappa, double lamdaP_a,double lamdaP_s,double lamdaP_N,const Matrix3d &CC,double rho, double c, Matrix3d &SS_pas,Matrix3d &SS_act, Matrix3d&SS_vol)
{
    ...
    double Psivol = 0.5*phif*pow(kv*(Je-1.),2) - 2*phif*k0*log(Je);
    double dPsivoldJe = phif*kv*(Je-1.) - 2*phif*k0/Je;
    double dPsivoldJedJe = phif*kv + 2*k0/(Je*Je);
    Matrix3d SSe_vol = dPsivoldJe*Je*CCeinv/2;
...
void evalBC(int surface_boundary_flag, const std::vector<double> &ip_Jac, const std::vector<double> &global_parameters,
    ...
    } else if (surf_elem_size == 3) {
        // linear hexahedron
        IP = LineQuadriIPSurfaceTri();
    } else{
        throw std::runtime_error("Wrong number of nodes in element!");
    }
    int IP_size = IP.size();
    ...
    for (int ip = 0; ip < IP_size; ip++) {
        ...
        if(surf_elem_size == 4){
            ...
            R = evalShapeFunctionsSurfaceTriR(xi,eta);
            Rxi = evalShapeFunctionsSurfaceTriRxi(xi,eta);
            Reta = evalShapeFunctionsSurfaceTriReta(xi,eta);
        } else{
            throw std::runtime_error("Wrong number of nodes in element!");
        }
...
//            std::cout << "\n" << Jac << rho_flux << R[nodei] << wip << "\n";
//            std::cout << "\n" << Jac << c_flux << R[nodei] << wip << "\n";
//            std::cout << "\n" << Jac << k_rho << rho_inf << rho << R[nodei] << wip << "\n";
//            std::cout << "\n" << Jac << k_c << c_inf << c << R[nodei] << wip << "\n";
        }
            ...
            for(int coordi=0;coordi<3;coordi++){
                for(int nodej=0;nodej<surf_elem_size;nodej++){
                    for(int coordj=0;coordj<3;coordj++){
                        ...
                        // This may not be right, but I set the spring to 0 anyway
        ...
        //---------------------------------//
//        std::cout << " Re rho Re c\n" << Re_rho_surf << "\n" << Re_c_surf << "\n";
//        std::cout << " Ke_rho_rho_surf Ke_rho_c_surf\n" << Ke_rho_rho_surf << "\n" << Ke_rho_c_surf << "\n";
//        std::cout << "Ke_c_c_surf Ke_c_rho_surf\n" << Ke_c_c_surf << "\n" << Ke_c_rho_surf << "\n";
    }
}
