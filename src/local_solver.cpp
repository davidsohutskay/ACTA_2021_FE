...
//        lamdaP_dot(2) = phif_dot_plus*(lamdaE_n-1)/tau_lamdaP_n;
        ...
        // lamdaP_a
        if(lamdaE_a < 0.995){
            lamdaP_dot(0) = phif_dot_plus*(lamdaE_a-0.995)/tau_lamdaP_a;
        } else if(lamdaE_a > 1.005){
            lamdaP_dot(0) = phif_dot_plus*(lamdaE_a-1.005)/tau_lamdaP_a;
        }
        else{
            lamdaP_dot(0) = 0;
        }
        // lamdaP_s
        if(lamdaE_s < 0.995){
            lamdaP_dot(1) = phif_dot_plus*(lamdaE_s-0.995)/tau_lamdaP_s;
        } else if(lamdaE_s > 1.005){
            lamdaP_dot(1) = phif_dot_plus*(lamdaE_s-1.005)/tau_lamdaP_s;
        }else{
            lamdaP_dot(1) = 0;
        }
        // lamdaP_n
        if(lamdaE_n < 0.995){
            lamdaP_dot(2) = phif_dot_plus*(lamdaE_n-0.995)/tau_lamdaP_n;
        } else if(lamdaE_n > 1.005){
            lamdaP_dot(2) = phif_dot_plus*(lamdaE_n-1.005)/tau_lamdaP_n;
        }else{
            lamdaP_dot(2) = 0;
        }
...
//            dThetadCC(42+II) += (local_dt/tau_lamdaP_n)*((dphifdotplusdCC(ii,jj)*(lamdaE_n-1)) + (phif_dot_plus*(dlamdaE_n_dCC(ii,jj))));
            // lamdaP_a
            if(lamdaE_a < 0.995){ // && (lamdaE_a < 1.05 && lamdaE_a > 0.95)
                dThetadCC(30+II) += (local_dt/tau_lamdaP_a)*((dphifdotplusdCC(ii,jj)*(lamdaE_a-0.995)) + (phif_dot_plus*(dlamdaE_a_dCC(ii,jj))));
            } else if(lamdaE_a > 1.005){ // && (lamdaE_a < 1.05 && lamdaE_a > 0.95)
                dThetadCC(30+II) += (local_dt/tau_lamdaP_a)*((dphifdotplusdCC(ii,jj)*(lamdaE_a-1.005)) + (phif_dot_plus*(dlamdaE_a_dCC(ii,jj))));
            } else{
                dThetadCC(30+II) = 0;
            }
            // lamdaP_s
            if(lamdaE_s < 0.995){ //  && (lamdaE_s < 1.05 && lamdaE_s > 0.95)
                dThetadCC(36+II) += (local_dt/tau_lamdaP_s)*((dphifdotplusdCC(ii,jj)*(lamdaE_s-0.995)) + (phif_dot_plus*(dlamdaE_s_dCC(ii,jj))));
            } else if(lamdaE_s > 1.005){ //  && (lamdaE_s < 1.05 && lamdaE_s > 0.95)
                dThetadCC(36+II) += (local_dt/tau_lamdaP_s)*((dphifdotplusdCC(ii,jj)*(lamdaE_s-1.005)) + (phif_dot_plus*(dlamdaE_s_dCC(ii,jj))));
            } else{
                dThetadCC(36+II) = 0;
            }
            // lamdaP_n
            if(lamdaE_n < 0.995){ // && (lamdaE_n < 1.05 && lamdaE_n > 0.95)
                dThetadCC(42+II) += (local_dt/tau_lamdaP_n)*((dphifdotplusdCC(ii,jj)*(lamdaE_n-0.995)) + (phif_dot_plus*(dlamdaE_n_dCC(ii,jj))));
            } else if(lamdaE_n > 1.005){ // && (lamdaE_n < 1.05 && lamdaE_n > 0.95)
                dThetadCC(42+II) += (local_dt/tau_lamdaP_n)*((dphifdotplusdCC(ii,jj)*(lamdaE_n-1.005)) + (phif_dot_plus*(dlamdaE_n_dCC(ii,jj))));
            } else{
                dThetadCC(42+II) = 0;
            }
        }
...
//        dThetadrho(7) += local_dt*((lamdaE_n-1)/tau_lamdaP_n)*dphifdotplusdrho;
        ...
        // lamdaP_a
        if(lamdaE_a < 0.995){ // && (lamdaE_a < 1.05 && lamdaE_a > 0.95)
            dThetadrho(5) += local_dt*((lamdaE_a-0.995)/tau_lamdaP_a)*dphifdotplusdrho;
        } else if(lamdaE_a > 1.005){ // && (lamdaE_a < 1.05 && lamdaE_a > 0.95)
            dThetadrho(5) += local_dt*((lamdaE_a-1.005)/tau_lamdaP_a)*dphifdotplusdrho;
        }else{
            dThetadrho(5) = 0;
        }
        // lamdaP_s
        if(lamdaE_s < 0.995){ //  && (lamdaE_s < 1.05 && lamdaE_s > 0.95)
            dThetadrho(6) += local_dt*((lamdaE_s-0.995)/tau_lamdaP_s)*dphifdotplusdrho;
        } else if(lamdaE_s > 1.005){ //  && (lamdaE_s < 1.05 && lamdaE_s > 0.95)
            dThetadrho(6) += local_dt*((lamdaE_s-1.005)/tau_lamdaP_s)*dphifdotplusdrho;
        } else{
            dThetadrho(6) = 0;
        }
        // lamdaP_n
        if(lamdaE_n < 0.995){ // && (lamdaE_n < 1.05 && lamdaE_n > 0.95)
            dThetadrho(7) += local_dt*((lamdaE_n-0.995)/tau_lamdaP_n)*dphifdotplusdrho;
        } else if(lamdaE_n > 1.005){ // && (lamdaE_n < 1.05 && lamdaE_n > 0.95)
            dThetadrho(7) += local_dt*((lamdaE_n-1.005)/tau_lamdaP_n)*dphifdotplusdrho;
        } else{
            dThetadrho(7) = 0;
        }
...
//        dThetadc(7) += local_dt*((lamdaE_n-1)/tau_lamdaP_n)*dphifdotplusdc;
        ...
        // lamdaP_a
        if(lamdaE_a < 0.995){ // && (lamdaE_a < 1.05 && lamdaE_a > 0.95)
            dThetadc(5) += local_dt*((lamdaE_a-0.995)/tau_lamdaP_a)*dphifdotplusdc;
        } else if(lamdaE_a > 1.005){ // && (lamdaE_a < 1.05 && lamdaE_a > 0.95)
            dThetadc(5) += local_dt*((lamdaE_a-1.005)/tau_lamdaP_a)*dphifdotplusdc;
        } else{
            dThetadc(5) = 0;
        }
        // lamdaP_s
        if(lamdaE_s < 0.995){ // && (lamdaE_s < 1.05 && lamdaE_s > 0.95)
            dThetadc(6) += local_dt*((lamdaE_s-0.995)/tau_lamdaP_s)*dphifdotplusdc;
        } else if(lamdaE_s > 1.005){ // && (lamdaE_s < 1.05 && lamdaE_s > 0.95)
            dThetadc(6) += local_dt*((lamdaE_s-1.005)/tau_lamdaP_s)*dphifdotplusdc;
        } else{
            dThetadc(6) = 0;
        }
        // lamdaP_n
        if(lamdaE_n < 0.995){ // && (lamdaE_n < 1.05 && lamdaE_n > 0.95)
            dThetadc(7) += local_dt*((lamdaE_n-0.995)/tau_lamdaP_n)*dphifdotplusdc;
        } else if(lamdaE_n > 1.005){ // && (lamdaE_n < 1.05 && lamdaE_n > 0.95)
            dThetadc(7) += local_dt*((lamdaE_n-1.005)/tau_lamdaP_n)*dphifdotplusdc;
        } else{
            dThetadc(7) = 0;
        }
