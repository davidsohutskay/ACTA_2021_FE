...
void evalElemJacobiansSurface(tissue &myTissue)
{
    ...
    std::cout<<"evaluating element surface jacobians, over "<< myTissue.n_surf_elem <<" elements\n";
...
#pragma omp critical
            	for(int ipi=0;ipi<IP_size;ipi++){
                    ...
                    if(IP_size == elem_size){
                        ...
                        // If nIP != elem_size, we can average the element
                        for(int nodei=0;nodei<elem_size;nodei++){
                            node_phi[myTissue.vol_elem_connectivity[ei][nodei]] += ip_phif_pi[ipi];
                            node_dphifdu[myTissue.vol_elem_connectivity[ei][nodei]] += ip_dphifdu[ipi];
                            node_dphifdrho[myTissue.vol_elem_connectivity[ei][nodei]] += ip_dphifdrho[ipi];
                            node_dphifdc[myTissue.vol_elem_connectivity[ei][nodei]] += ip_dphifdc[ipi];
                            node_ip_count[myTissue.vol_elem_connectivity[ei][nodei]] += 1;
                        }
                    }
